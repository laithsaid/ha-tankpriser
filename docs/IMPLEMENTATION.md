...
| `sources.py` | `fetch_all`, then `apply_discounts` and the `PROVIDERS` dict | One `Provider(key, label, fetch)` per chain. `_one_shot(url, parser)` covers the simple ones — with `headers=` for a constant one a source demands, which is how Circle K gets its `X-App-Name`; OIL! needs `fetch_oil` because it is priced one fuel type per request. The cache is keyed by provider **and** a credential fingerprint, so changing a key bypasses it. |
...
- `tankpriser-prediction-card` — one compact panel per car (`entities: [...]`,
  ...
  `show_donate` or `donate_url` option any more, and an old config carrying one
  is ignored: the link comes from the sensor's `donate_url` attribute
  (i.e. `const.py`), falling back to the card's own copy of `DONATE_URL`. Its editor picks cars with a `multiple: true`
