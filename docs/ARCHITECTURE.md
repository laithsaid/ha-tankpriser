...
| --- | --- | --- | --- |
| Chain and regulator price APIs (OK, Q8/F24, Shell, OIL!, Circle K/INGO, Go'on, Uno-X; Tankerkönig for DE; ANWB for NL/BE/LU; prix des carburants for FR; E-Control for AT; MITECO for ES; Osservaprezzi for IT) | HA server | Prices | Server-side; honest `User-Agent`. All open except Go'on, whose personal key travels as an `Authorization` header and never in a URL; Uno-X, whose client_id/secret pair is spent only on a token request and never on the data call, with the 15-minute token cached per credential; and Tankerkönig, which takes its key as a query parameter — the only one that does, which is why `redact()` exists. Circle K also wants a constant `X-App-Name: PRICES` header, which is not a credential — it refuses the request without it. |
