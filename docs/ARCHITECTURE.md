...
| --- | --- | --- | --- |
| Chain and regulator price APIs (OK, Q8/F24, Shell, OIL!, Circle K/INGO, Go'on; Tankerkönig for DE; ANWB for NL/BE/LU/FR; E-Control for AT) | HA server | Prices | Server-side; honest `User-Agent`. All open except Go'on, whose personal key travels as an `Authorization` header and never in a URL, and Tankerkönig, which takes its key as a query parameter — the only one that does, which is why `redact()` exists. Circle K also wants a constant `X-App-Name: PRICES` header, which is not a credential — it refuses the request without it. |
