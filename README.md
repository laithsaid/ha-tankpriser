...
| You followed an earlier version of this guide, which used **Dictate Text** | Delete that action. Every action runs whether or not anything uses its output, so a leftover one still asks Siri for a microphone she is already holding. **First check where your index came from**: open **Get Item from List** and look at its **Index** field. If the chip says *Dictated Text*, deleting the action empties it — tap the field and pick **Provided Input** (from **Ask for Input**) instead. If it already says *Provided Input*, delete away. |
